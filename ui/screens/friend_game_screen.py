...
class FriendGameScreen(arcade.View):
    def __init__(self, word_p1: str, word_p2: str):
        super().__init__()

        self.background_tex = arcade.load_texture("data/images/background/background.png")

        self.grid_textures = {
            n: arcade.load_texture(f"data/images/grids/grid_{n}.png") for n in range(3, 10)}
        ...
        self.players = {
            0: self.create_player(word_p2),
            1: self.create_player(word_p1),
        }
    ...
    def create_player(self, target_word):
        ...
        return {
            ...
            "row": 0,
            "col": 0,
            "finished": False,  # Флаг завершения игрока
            "won": False,  # Флаг победы игрока
            "attempts_used": 0,  # Использованные попытки
            "grid": [
                [{"letter": "", "state": None} for _ in range(cols)]
                for _ in range(rows)
            ]
        }
    ...
    def draw_player_grid(self, idx, center_x):
        ...
        scale = 0.85
        gw = grid_tex.width * scale
        gh = grid_tex.height * scale
        cy = self.height / 2 + 80

        arcade.draw_texture_rect(
            grid_tex,
            arcade.rect.XYWH(center_x, cy, gw, gh)
        )
        ...
        for r in range(p["rows"]):
            for c in range(p["cols"]):
                ...
                x = sx + c * cell_w + cell_w / 2
                y = sy - r * cell_h - cell_h / 2
                ...
                if cell["state"] is None:
                    ...
                    tex = self.get_letter_texture(
                        cell["letter"], cell["state"]
                    )
                    arcade.draw_texture_rect(
                        tex,
